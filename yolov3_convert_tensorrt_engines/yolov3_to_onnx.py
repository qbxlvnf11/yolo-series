...
class GraphBuilderONNX(object):
    ...
    def __init__(self, output_tensors, batch_size, channels, resize_width_height):
    ...
    def build_onnx_graph(
        ...
        of YOLOv3), create an ONNX graph, populate it with weights from the weights
        ...
        for i, layer_name in enumerate(self.param_dict.keys()):
    ...
    def _make_input_tensor(self, layer_name, layer_dict):
        ...
        input_tensor = helper.make_tensor_value_info(
            str(layer_name), TensorProto.FLOAT, [
                self.batch_size, self.channels, self.resize_width_height, self.resize_width_height])
        self.input_tensor = input_tensor
        return layer_name, self.channels
...
def yolov3_to_onnx(config_dict):
    ...
    weight_path = yolov3_config.get('yolov3', 'weight_path', raw=True)
    batch_size = 1
    channels = yolov3_config.getint('yolov3', 'yolov3_channels')
    ...
    supported_layers = ['convolutional', 'shortcut',
                        'route', 'upsample']
    ...
    layer_configs = parser.parse_cfg_file(model_def_config_path)
    ...
    output_tensor_dims[conv_layers[0]] = [(class_num + 5) * 3, resize_width_height // 32, resize_width_height // 32]
    output_tensor_dims[conv_layers[1]] = [(class_num + 5) * 3, resize_width_height // 16, resize_width_height // 16]
    output_tensor_dims[conv_layers[2]] = [(class_num + 5) * 3, resize_width_height // 8, resize_width_height // 8]
    print('output_tensor_dims:', output_tensor_dims)

    # Create a GraphBuilderONNX object with the known output tensor dimensions:
    builder = GraphBuilderONNX(output_tensor_dims, batch_size, channels, resize_width_height)
    ...
    onnx.save(yolov3_model_def, save_onnx_model_path)
